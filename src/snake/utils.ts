...
function replyMarkupInlineKeyboard(replyMarkup:Interface.inlineKeyboard) {
  ...
  for(let row = 0; row < replyMarkup.inlineKeyboard.length; row++){
    ...
    for(let col = 0; col < replyMarkup.inlineKeyboard[row].length; col ++){
      let btn:Interface.inlineKeyboardButton = replyMarkup.inlineKeyboard[row][col] as Interface.inlineKeyboardButton
      // button url
      if(btn.url){
        tempCol.push(new Api.KeyboardButtonUrl({
      ...
      // button login url 
      if(btn.loginUrl){
        tempCol.push(new Api.InputKeyboardButtonUrlAuth({
          text : String(btn.text),
      ...
      // button callbackData 
      if(btn.callbackData){
        tempCol.push(new Api.KeyboardButtonCallback({
          text : String(btn.text),
      ...
      if(btn.switchInlineQuery){
        tempCol.push(new Api.KeyboardButtonSwitchInline({
          text : String(btn.text),
          samePeer : false,
      ...
      // button game 
      if(btn.callbackGame){
        tempCol.push(new Api.KeyboardButtonGame({
          text : String(btn.text)
        }))
        continue;
      }
      // button buy 
      if(btn.buy){
        tempCol.push(new Api.KeyboardButtonBuy({
          text : String(btn.text)
        }))
        continue;
      }
  ...
  for(let row = 0; row < replyMarkup.keyboard.length; row ++){
    ...
    for(let col = 0; col < replyMarkup.keyboard[row].length; col ++){ 
      ...
      if(typeof replyMarkup.keyboard[row][col] == "string"){
        tempCol.push(new Api.KeyboardButton({
          text : String(replyMarkup.keyboard[row][col])
        }))
        continue;
      }
      if(typeof replyMarkup.keyboard[row][col] !== "string"){ 
        let btn:Interface.replyKeyboardButton = replyMarkup.keyboard[row][col] as Interface.replyKeyboardButton
        // keyboard requestContact 
        if(btn.requestContact){
          tempCol.push(new Api.KeyboardButtonRequestPhone({
            text : String(btn.text)
          }))
          continue;
        }
        ...
        if(btn.requestLocation){
          tempCol.push(new Api.KeyboardButtonRequestGeoLocation({
            text : String(btn.text)
          }))
          continue;
        }
        ...
        if(btn.requestPoll){
          tempCol.push(new Api.KeyboardButtonRequestPoll({
            text : String(btn.text),
